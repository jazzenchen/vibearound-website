...
| Channel | Setup page |
|---|---|
| Telegram | [channels/telegram](/docs/channels) |
| Feishu / Lark | [channels/feishu](/docs/channels) — permission JSON, long-connection subscriptions |
| Slack | [channels/slack](/docs/channels) — one-paste app manifest, Socket Mode |
| Discord | [channels/discord](/docs/channels) |
| DingTalk | [channels/dingtalk](/docs/channels) |
| WeCom | [channels/wecom](/docs/channels) |
| QQ Bot | [channels/qqbot](/docs/channels) |
| WeChat | [channels/wechat](/docs/channels) — QR login |
| WhatsApp | [channels/whatsapp](/docs/channels) — QR login |

Kind ids and repository links are also in the [supported matrix](/docs/supported-agents). Each plugin README additionally documents:
...
Apply with `va channel sync` ([the pattern](/docs/channels)). The optional `verbose` object works on every channel: `show_thinking` and `show_tool_use` (both default `false`) include the agent's thinking and tool-use blocks in chat output.
...
- Slack reserves bare slash commands, so VibeAround commands use the `/va` prefix: `/va new`, `/va switch claude`, `/va status` ([command reference](/docs/remote-messaging-terminal) — every command works behind the prefix).
