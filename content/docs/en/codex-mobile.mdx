---
title: Codex CLI From Phone
description: Use VibeAround to keep local Codex CLI sessions reachable from a phone-friendly browser, Web Terminal, handover, or messaging channel.
---
...
Use this page when the goal is to control or inspect a local Codex CLI workflow from another device. If you are comparing hosted Codex workflows, official mobile access, SSH, tunnels, and VibeAround's approach, where execution stays on your own computer, read [Codex Remote Comparison](/docs/codex-remote) after this setup path.
...
3. Launch or continue a Codex session from [Agent Launch](/docs/agent-launch).
4. Open the session from a mobile browser with [Session Handover](/docs/session-handover).
5. Add [Remote Messaging & Web Terminal](/docs/remote-messaging-terminal) only after the local session works.
6. Use [Live Preview](/docs/live-preview) to review local outputs from the same phone-friendly flow.
...
- [Codex Remote Comparison](/docs/codex-remote)
