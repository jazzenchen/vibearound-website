...
Fifteen minutes, three "aha" moments: chat with a local agent in the browser, do the same from an IM app, then move one conversation between surfaces. Prerequisite: VibeAround installed with at least one agent enabled ([Install and onboarding](./installation)).
...
2. Add the channel in the desktop app (or `settings.json` → `channels.telegram`, then `va channel sync`) — details in [Connect channels](./channels).
...
| You want to… | Read |
|---|---|
| Understand what just happened (threads, routes, sessions) | [Concepts](./unified-workspace) |
| Use one provider subscription across agent CLIs | [Model profiles guide](./provider-profiles) |
| Reach the dashboard away from home | [Tunnels and remote access](https://github.com/jazzenchen/VibeAround/blob/main/docs/guides/tunnels-and-remote-access.md) |
| The full slash-command list | [IM usage](./remote-messaging-terminal) |
