...
Per-channel setup pages (platform steps + verified config blocks) live under [guides/channels/](/docs/channels); configuration lives under `channels.<kind>` in `settings.json`.
...
Plan-by-plan detail — endpoint groups, regions, base URLs, model sets, and which credential each plan expects — is in the [provider endpoints reference](/docs/providers).
