...
Share links expire after 600 seconds by design. Re-share, or use the owner link where you are authenticated. See [Security model](./local-first-security).

## Models and profiles

**The agent rejects the model name.**
Use the profile's alias model id (agents often validate model names against their vendor's list). The [Model profiles guide](./provider-profiles) has a fuller table for 401s, capability mismatches, and rate limits.

## Platform quirks

- **macOS Intel:** source-build only for the desktop app ([Build from source](./develop-locally)).
